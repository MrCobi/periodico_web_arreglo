/*
  ...
  - You are about to drop the `FavoriteSource` table. If the table is not empty, all the data it contains will be lost.
...
-- DropForeignKey
ALTER TABLE `FavoriteSource` DROP FOREIGN KEY `FavoriteSource_sourceId_fkey`;

-- DropForeignKey
ALTER TABLE `FavoriteSource` DROP FOREIGN KEY `FavoriteSource_userId_fkey`;
...
-- DropForeignKey
ALTER TABLE `Rating` DROP FOREIGN KEY `Rating_sourceId_fkey`;
...
-- AddForeignKey
ALTER TABLE `favorite_sources` ADD CONSTRAINT `favorite_sources_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
