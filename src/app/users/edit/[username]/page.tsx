...
export default function EditUserPage() {
  ...
  useEffect(() => {
    ...
    if (session.user.username !== username) {
      ...
      console.log("Session username:", session.user.username, "| URL username:", username);
      setLoading(false);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/users/${username}`, { // API URL corregida
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-[#0D1117] p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-[#161B22] p-6 rounded-lg">
