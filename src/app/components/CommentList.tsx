...
interface Comment {
  ...
  user: {
    id: string;
    username: string; // Cambiar name por username
    image: string | null;
  };
  parent?: {
    user: {
  ...
  ({
    ...
    return (
      <div className={`${
        ...
        <div className={`bg-white dark:bg-gray-800 p-3 md:p-4 rounded-xl shadow-md relative ${
          isNewComment ? "animate-pulse-light border-2 border-blue-200 dark:border-blue-800" : ""
        }`}>
          ...
          <div className="mt-2 ml-2 md:ml-4">
            ...
            >
              <svg 
                ...
                className={`h-3.5 w-3.5 md:h-4 md:w-4 transition-transform duration-300 ${isVisible ? "rotate-90" : "group-hover:translate-x-0.5"}`} 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
              ...
              {isVisible ? "Ocultar respuestas" : `${comment.replies?.length} ${comment.replies?.length === 1 ? "respuesta" : "respuestas"}`}
            </button>
          </div>
        )}
